...
function isRegisterResponse(obj: unknown): obj is { email: string; name: string } {
...
async function parseSessionSafe(res: Response): Promise<Session> {
    try {
        ...
        const { id, email, name } = json as { id?: unknown; email?: unknown; name?: unknown };
        return typeof id === "number" && typeof email === "string" && typeof name === "string"
            ? { id, email, name }
            : null;
    } catch (error) {
